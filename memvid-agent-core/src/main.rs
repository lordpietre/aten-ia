...
fn main() -> Result<()> {
    ...
    let _lock = FileLock::acquire(&config.data_dir)
        .expect("Failed to acquire data directory lock");
...

    {
        ...
        eprintln!("{} {} knowledge entries indexed", "↳".dimmed(), a.knowledge_count());
        if is_first_run {
    ...
    loop {
        ...
        if input_lower == "/models" || input == "/MODELS" {
            ...
            for entry in catalog.list() {
                let current = if entry.name == config.model.name { " ◄ active" } else { "" };
                println!(
                    "  {:<20} {} ({} MB){}",
        ...
        if let Some(model_id) = input.strip_prefix("/model ") {
            ...
            let entry = match catalog.find(model_id) {
                Some(e) => e,
                None => {
                    eprintln!("{} Unknown model '{}'. Use /models to list available.", "✗".red(), model_id);
                    continue;
                }
            };
        ...
        if input_lower == "/languages" || input == "/LANGUAGES" {
            ...
            println!("{} Available Languages ({})", "━━━ Languages ━━━".bold(), lang_catalog.len());
            ...
            if lang_catalog.len() > 30 {
                println!("  ... and {} more (use /learn <lang> to fetch docs)", lang_catalog.len() - 30);
            }
            continue;
        }
        ...
        if input_lower == "/books" || input_lower == "/BOOKS" {
            ...
            println!("{} Free Programming Books - Languages ({})", "━━━ Books ━━━".bold(), langs.len());
        ...
        if let Some(lang_id) = input.strip_prefix("/learn ") {
            ...
            let lang = match lang_catalog.find(&lang_id) {
                Some(l) => l,
                None => {
                    eprintln!("{} Unknown language '{}'. Use /languages to list available.", "✗".red(), lang_id);
                    continue;
                }
            };
            ...
            let entries =
                memvid_agent_core::languages_catalog::download_language_resources(lang, max_resources)?;
        ...
        if input_lower.starts_with("/search ") {
            let query = input.strip_prefix("/search ").or_else(|| input.strip_prefix("/SEARCH ")).unwrap_or("").trim().to_string();
            if query.is_empty() {
        ...
        if input_lower.starts_with("/load ") {
            let path = input.strip_prefix("/load ").or_else(|| input.strip_prefix("/LOAD ")).unwrap_or("").trim();
        ...
        if input_lower.starts_with("/ingest ") {
            let path = input.strip_prefix("/ingest ").or_else(|| input.strip_prefix("/INGEST ")).unwrap_or("").trim();
            ...
            if is_binary {
                match agent.lock().unwrap().ingest_file(file_path) {
                    Ok(extracted) => {
                        ...
                        println!("{} Ingested: {} ({} format, {} chars)", "✓".green(), title.bold(), extracted.format, extracted.content.len());
                    }
        ...
        if input_lower.starts_with("/ingest-pdf ") || input_lower.starts_with("/INGEST-PDF ") {
            let path = input.strip_prefix("/ingest-pdf ").or_else(|| input.strip_prefix("/INGEST-PDF ")).unwrap_or("").trim();
            let file_path = std::path::Path::new(path);
            match agent.lock().unwrap().ingest_file(file_path) {
                Ok(extracted) => {
                    let title = extracted.title.as_deref().unwrap_or("untitled");
        ...
        if input_lower.starts_with("/fetch ") {
            let url = input.strip_prefix("/fetch ").or_else(|| input.strip_prefix("/FETCH ")).unwrap_or("").trim();
            if url.is_empty() {
            ...
            match result {
                Ok(content) => {
                    ...
                    println!("{} Fetched: {} ({})", "✓".green(), title.bold(), url.dimmed());
        ...
        if input_lower.starts_with("/fetch-md ") {
            let url = input.strip_prefix("/fetch-md ").or_else(|| input.strip_prefix("/FETCH-MD ")).unwrap_or("").trim();
            if url.is_empty() {
            ...
            let result = {
                let mut a = agent.lock().unwrap();
                let mut fetcher = memvid_agent_core::web_fetcher::WebFetcher::new(&config.ingestion);
            ...
            match result {
                Ok(content) => {
                    ...
                    let title = content.title.as_deref().unwrap_or("untitled");
                    println!("{} {} — Markdown ({} chars)", "━━━".bold(), title.bold(), md.len());
                    println!("{}", md.dimmed());
                }
                Err(e) => {
                    eprintln!("{} Failed to fetch: {:#}", "✗".red(), e);
        ...
        if input_lower.starts_with("/batch ") {
            let path = input.strip_prefix("/batch ").or_else(|| input.strip_prefix("/BATCH ")).unwrap_or("").trim();
            if path.is_empty() {
            ...
            println!("{} Processing {} URLs from {}…", "↓".yellow(), urls.len(), path.dimmed());
            ...
            match result {
                Ok(stats) => {
                    println!("{} Batch complete: {} ok, {} failed, ~{} chunks",
                        "✓".green(), stats.success.len(), stats.failures.len(), stats.total_chunks);
        ...
        if input_lower.starts_with("/download-books ") || input_lower.starts_with("/DOWNLOAD-BOOKS ") {
            let rest = input.strip_prefix("/download-books ").or_else(|| input.strip_prefix("/DOWNLOAD-BOOKS ")).unwrap_or("").trim();
            if rest.is_empty() {
            ...
            println!("{} Downloading and ingesting up to {} resources for '{}'…", "↓".yellow(), limit, lang.bold());
            let spinner = indicatif::ProgressBar::new_spinner();
            spinner.set_style(
                indicatif::ProgressStyle::default_spinner()
                    .template("{spinner:.cyan} {msg}")
            ...
            match result {
                Ok(count) => {
                    ...
                    if let Err(e) = config.save() { eprintln!("{} Failed to save config: {}", "✗".red(), e); }
                    println!("{} Ingested {} resource(s) for '{}'.", "✓".green(), count, lang.bold());
                }
                Err(e) => {
...
fn print_startup_help() {
    ...
    eprintln!("  {:<20}  List / switch models", "/models | /model <id>".dimmed());
...
fn print_help() {
    ...
    println!("  {:<20}  Show / download / switch model", "/model [id|current]".bright_blue());
    println!("  {:<20}  List available languages", "/languages".bright_blue());
    println!("  {:<20}  List installed languages", "/languages-installed".bright_blue());
    println!("  {:<20}  List programming-book languages (free-programming-books)", "/books".bright_blue());
    println!("  {:<20}  Download & ingest books for a language", "/download-books <lang> [limit]".bright_blue());
    println!("  {:<20}  Download docs for a language", "/learn <lang>".bright_blue());
    println!("  {:<20}  Remove language docs from knowledge", "/unlearn <lang>".bright_blue());
    println!("  {:<20}  Fetch URL and index as knowledge", "/fetch <url>".bright_blue());
    println!("  {:<20}  Fetch URL and show markdown", "/fetch-md <url>".bright_blue());
    println!("  {:<20}  Process batch of URLs from file", "/batch <file>".bright_blue());
    println!("  {:<20}  Load a text file into session context", "/load <file>".bright_blue());
    println!("  {:<20}  Load file and index as knowledge (auto-detects PDF/EPUB)", "/ingest <file>".bright_blue());
    println!("  {:<20}  Extract & index a PDF/EPUB file", "/ingest-pdf <file>".bright_blue());
    println!("  {:<20}  Search indexed knowledge", "/search <query>".bright_blue());
    println!("  {:<20}  Rebuild knowledge index from .mv2 files", "/reindex".bright_blue());
    println!("  {:<20}  Start API server / show token", "/token".bright_blue());
    println!("  {:<20}  Show conversation history", "/history".bright_blue());
    println!("  {:<20}  Show agent statistics", "/stats".bright_blue());
    println!("  {:<20}  Show current configuration", "/config".bright_blue());
    println!("  {:<20}  Show this help", "/help".bright_blue());
    println!("  {:<20}  Exit the agent", "/exit".bright_blue());
    println!();
    println!("{} Commands also accept uppercase: /MODELS, /MODEL, /LANGUAGES, /SEARCH, /REINDEX, /TOKEN, /LOAD, /INGEST", "↳".dimmed());
    println!();
}
...
fn print_stats(
    ...
    println!("  {} {} interactions", "Chats:".dimmed(), agent.interaction_count());
...
fn print_config(config: &Config) {
    ...
    println!("  {} {}", "developer_prompt:".dimmed(), config.developer_prompt.as_deref().unwrap_or("(default)"));
    ...
    println!("    {} {}", "n_gpu_layers:".dimmed(), config.model.n_gpu_layers);
    println!("    {} {}", "chat_template:".dimmed(), config.model.chat_template);
    ...
    println!("    {} {}", "max_tokens:".dimmed(), config.generation.max_tokens);
    println!("  {} API", "──".dimmed());
    println!("    {} {}", "enabled:".dimmed(), config.api.enabled);
    println!("    {} {}", "host:".dimmed(), config.api.host);
    println!("    {} {}", "port:".dimmed(), config.api.port);
    println!("    {} {}", "token:".dimmed(), config.api.token.as_deref().unwrap_or("(none)"));
...
fn run_setup_wizard(config: &mut Config, catalog: &ModelsCatalog) -> Result<()> {
    println!();
    println!("{} Welcome to memvid-agent-core!", "━━━ Setup ━━━".bold().bright_green());
    ...
    let model_choice = read_line_prompt(&format!(
        "{} Select model [1]: ",
        "•".dimmed()
    ));
    ...
    if model_idx > 0 && model_idx <= catalog.list().len() {
        ...
        if models_catalog::download_model(entry, models_dir).is_ok() {
            ...
            println!("  {} Model download deferred — will download on next launch.", "i".yellow());
        }
    }
        ...
        match LanguagesCatalog::load_or_fetch(&config.data_dir) {
            Ok(lang_catalog) => {
                ...
                if lang_catalog.len() > 5 {
                    println!("     ... and {} more (install later with /learn)", lang_catalog.len() - 5);
                }
                let lang_choice = read_line_prompt("  Enter language key (or leave blank to skip): ");
                if !lang_choice.is_empty() && lang_catalog.find(&lang_choice).is_some() {
                    println!("  {} Language '{}' will be installed on next /learn", "↳".dimmed(), lang_choice);
                }
            }
            Err(e) => {
    ...
    let enable_api = read_line_prompt(&format!(
        "{} Enable API server? (y/N): ",
        "•".dimmed()
    ));
    ...
    config.save()?;
    println!();
    println!("{} Setup complete! Configuration saved.", "✓".green().bold());
    println!();
    Ok(())
}
